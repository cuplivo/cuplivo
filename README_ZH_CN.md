...
> **项目状态（v4.0.0 —— 重定基线）**
>
> Cuplivo 回来了：v4.0.0 以**重放（绝不 merge）**方式将 fork 基线迁至上游 Kelivo `915a8b1d`（决策记录见 `website/docs/adr/0004`）。全部 fork 功能现在运行在上游地基之上——Drift 数据库（v1→v6 迁移链）、schema 治理、备份可移植性——同时保持产品身份、产物命名契约与应用 ID `com.cup11.cuplivo` 不变。
>
> 已在新基线重放：消息回复、群聊、OCR+文档理解、Ta 的来信（主动关怀）、增量备份+局域网同步（全量重设计移植）、世界书分组、翻译可见语言管理、多 AI 对比（schema v6 `subgroupId` + `contextTokens`），以及工具线（临时会话保存、输入草稿、启动助手置顶、推理力度词汇、统计维度筛选、图像生成选项、AI 日志分析、Markdown 子序列复制、公式导出）。
>
> 已接受的损失（ADR 中记录）：旧回收站/墓碑数据不迁移；局域网同步历史在 v4 重置（增量引擎为全量重设计）。
>
> v3.x 线以 `cuplivo` 存档分支存续。开发在本线继续。
