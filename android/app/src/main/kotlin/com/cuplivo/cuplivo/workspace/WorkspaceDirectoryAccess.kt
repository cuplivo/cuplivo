package com.cuplivo.cuplivo.workspace
...
class WorkspaceDirectoryAccess(private val context: Context) {
    ...
    fun requestStorageAccess(result: MethodChannel.Result) {
        val activity = attachedActivity ?: run {
            result.error("foreground_activity_required", "Open Cuplivo to grant storage access.", null)
            return
        }
        if (hasStorageAccess()) {
            result.success(true)
    ...
    fun pick(result: MethodChannel.Result) {
        val activity = attachedActivity ?: run {
            result.error("foreground_activity_required", "Open Cuplivo to choose a folder.", null)
            return
        }
        if (!hasStorageAccess()) {
