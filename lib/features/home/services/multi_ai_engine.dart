...
import 'package:Cuplivo/features/home/services/message_pipeline.dart';
...
class MultiAIEngine extends ChangeNotifier {
  ...
  Future<void> _executeThreads({
    ...
    for (int i = 0; i < _models.length; i++) {
      ...
      if (await _chatController.appendPersistedTailMessage(assistantMessage)) {
      ...
      final stored = await _storedMessage(convId, assistantMessage.id);
  ...
  Future<String> startRound({
    ...
    if (await _chatController.appendPersistedTailMessage(userMessage)) {
      _chatController.notifyListeners();
    }
    _chatController.notifyListeners();
    ...
    final completeMessages = await _chatController
  ...
  Future<void> retryThread({
    ...
    try {
      ...
      final stored = await _storedMessage(conversation.id, newMessage.id);
      if (stored != null && !stored.isStreaming) {
  ...
  Future<void> retryRound({
    ...
    try {
      for (int i = 0; i < _threadIds.length; i++) {
        ...
        final stored = await _storedMessage(conversation.id, newMsg.id);
        if (stored != null && !stored.isStreaming) {
  ...
  Future<ChatMessage?> _storedMessage(
    String conversationId,
    String messageId,
  ) async {
    final persisted = await _chatService.repo.getMessage(messageId);
    if (persisted != null ||
  ...
  Future<void> addModelsAndExecute({
    ...
    var completeMessages = await _chatController
        .messagesForCompleteHistoryContext(conversation);
    // Bound the replay window at the anchor user message (inclusive); a
    // newer turn's history must not leak into this round's requests.
    if (anchorId != null) {
      final anchorIdx = completeMessages.indexWhere(
