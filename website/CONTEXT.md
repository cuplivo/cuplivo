...
**恢复工具（Recovery Tool）**:
合并**助手找回**与**对话找回**的单一工具：上传 Kelivo 备份 zip，一次扫描完成"缺失助手重建 + 孤儿消息重建会话壳 + null 会话挂载"。旧独立入口 `kelivo-fill-assistant.html` 是此工具的祖先前身，已并入恢复工具。
...
- 官网工具路由通过 `packages/core`（纯 TS、零 DOM）消费诸能力，UI 层不再重复实现
...
>
> **Dev:** "官网能加个后端做下载统计吗？"
> **Domain expert:** "可以——统计是公共只读数据。但如果哪天想'云端存用户备份'，不行：工具核心处理永远在浏览器本地（后端边界）。"
