...
class LanSyncServer extends ChangeNotifier {
  ...
  LanSyncServer({
    ...
    Future<List<String>> Function()? assistantIds,
  }) : _assistantIdsFn = assistantIds;

  final Future<List<String>> Function()? _assistantIdsFn;

  Future<List<String>> _assistantIds() async =>
      await (_assistantIdsFn?.call() ?? const <String>[]);
  ...
  Future<void> _handlePlan(HttpRequest request) async {
    ...
    final myAssistantIds = await _assistantIds();
  ...
  Future<void> _handleExchange(HttpRequest request) async {
    ...
    if (hasChatOrFileDelta || forceSettingsExchange) {
      ...
      myZip = await _dataSync.exportIncrementalToFile(incremental);
    }
  ...
  Future<SyncPlan> _computePlan({
    ...
    for (final entry in initiatorIndex.conversations.entries) {
      ...
      final myMsgIds = (await _chatService.repo.getMessageIds(convId));
      plans.add(
        computeConvPlan(
          ConvPlanInput(
            conversationId: convId,
    ...
    // Pre-warm the fork-point timestamps asynchronously (the repo is async
    // on this stack); the pure planner below reads the warmed cache.
    final forkTsCache = <String, DateTime?>{};
    for (final plan in plans) {
      final forkId = plan.forkPointMessageId;
      if (forkId == null || forkTsCache.containsKey(plan.conversationId)) {
        continue;
      }
      forkTsCache[plan.conversationId] = (await _chatService.repo.getMessage(
        forkId,
      ))?.timestamp;
    }
    DateTime? resolveFork(String convId, String forkId) => forkTsCache[convId];
