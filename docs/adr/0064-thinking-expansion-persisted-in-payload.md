...
- One shared `StreamController` helper (`buildReasoningSegmentsJson` + `persistReasoningSegments`) serves both `HomePageController` (mobile/desktop/Multi-AI/web viewport) and group chat; the `ChatController` list copy is updated in lockstep so an in-place re-restore reads the same payload as the database.
- Out of scope (unchanged): the legacy plain `reasoningText` fallback keeps its hardcoded `expanded=false` on restore, as does the legacy inline-`<think>` widget-local map.
