...
void Webview::PrintToPdf(const std::string& path, bool print_backgrounds,
                         double margin_inches, double page_width_mils,
                         double page_height_mils,
                         PrintToPdfCallback callback) {
  if (!IsValid()) {
    callback(false, "webview is not initialized");
    return;
  }
  auto env6 = host_->environment().try_query<ICoreWebView2Environment6>();
  if (!env6) {
    callback(false, "webview environment does not support print settings");
    return;
  }
  wil::com_ptr<ICoreWebView2PrintSettings> print_settings;
  HRESULT hr = env6->CreatePrintSettings(print_settings.put());
  if (FAILED(hr)) {
    callback(false, "failed to create print settings");
    return;
  }
  if (print_backgrounds) {
    print_settings->put_ShouldPrintBackgrounds(TRUE);
  }
  print_settings->put_Orientation(COREWEBVIEW2_PRINT_ORIENTATION_PORTRAIT);
  print_settings->put_ScaleFactor(1.0f);
  print_settings->put_MarginTop(margin_inches);
  print_settings->put_MarginBottom(margin_inches);
  print_settings->put_MarginLeft(margin_inches);
  print_settings->put_MarginRight(margin_inches);
  print_settings->put_PageWidth(page_width_mils / 1000.0);
  print_settings->put_PageHeight(page_height_mils / 1000.0);
  auto webview7 = webview_.try_query<ICoreWebView2_7>();
  if (!webview7) {
    callback(false, "webview runtime does not support print");
    return;
  }
  hr = webview7->PrintToPdf(
      util::Utf16FromUtf8(path).c_str(), print_settings.get(),
      Callback<ICoreWebView2PrintToPdfCompletedHandler>(
          [callback](HRESULT result, BOOL success) {
            if (success) {
              callback(true, std::string());
              return S_OK;
            }
            std::ostringstream message;
            message << "print to pdf failed with HRESULT 0x" << std::hex
                    << result;
            callback(false, message.str());
            return S_OK;
          })
          .Get());
  if (FAILED(hr)) {
    callback(false, "PrintToPdf failed to start");
  }
}
