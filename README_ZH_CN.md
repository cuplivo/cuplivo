...
1. **灵活的文件系统操作** — 提供重量级沙箱运行环境与轻量级文件系统访问：
   - **沙箱运行环境** — 完整 Linux 沙箱：**Android** 可在界面选择发行版，可在工作区设置中打开类 Termux 的交互式终端（与模型 Shell 工具相互独立），还可通过 SAF + 定时同步将外部目录挂载到工作区；**iOS** 通过 iSH 运行沙箱；完成相应配置的用户可以执行命令行工具。Android 另支持沙箱会话持久化——退出终端后保持运行、终端保活、软件启动时自动打开沙箱，让 MCP 服务器、网关等常驻服务随沙箱一同启动。
   - **轻量级文件系统访问** — 工作区本地工具直接读、写、正则查找本地文件，支持将本地目录挂载为工作区或外部挂载，无需命令行，安全性优先；可在应用内文件浏览器中浏览，grep 搜索结果支持分页与上下文，支持查看代码架构、下载互联网资源至工作区，长网页支持工作区缓存续读。电脑端支持自定义工作区目录位置，工作区文件支持在外部打开与分享。
...
1. **子代理委派 (Handoff)** — 通过本地工具 `kelivo_handoff` 将子任务委派给其他助手执行：子代理完成后将完整结果返回给主代理进一步处理，主对话附有实时进度看板，支持同轮并发调用。
...
10. **快捷指令** — 将“快捷短语”与“指令注入”合并为统一的“快捷指令”：支持系统提示词、用户消息前 / 后、进入输入框四种注入位置，单轮 / 会话持久激活。

11. **临时对话保存** — 临时对话可一键保存为正式对话并持久化到历史记录。

#### 界面渲染功能增强

1. **实验性 Web 对话视图** — 启用实验开关后，Android/iOS/macOS/Windows 的正常对话可切换为 WebView 渲染，并支持导入声明式 "Web 对话样式库" JSON 样式，定制气泡与卡片外观。

2. **多气泡渲染** — 可开启设置项，助手回复按空行分段显示为多个独立气泡，提升长篇回复阅读体验。

3. **数学公式渲染增强** — 内置 flutter_math_fork 修复 `ovalbox`、`operatorname` 与 `\tag` 渲染错误。

4. **阅读模式** — 较长的助手回答可开启阅读模式，降低阅读疲劳。

5. **SVG 预览** — 在 `svg` 代码块内联渲染 SVG 图表。

6. **预设消息** — 预设消息在聊天列表中折叠到切换栏后；仅存在预设消息时阻止新建对话。
