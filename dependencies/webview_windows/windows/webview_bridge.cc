...
constexpr auto kMethodPrintToPdf = "printToPdf";
...
void WebviewBridge::HandleMethodCall(
  ...
  // printToPdf: {path, printBackgrounds, marginInInches, pageWidthMils,
  // pageHeightMils}
  if (method_name.compare(kMethodPrintToPdf) == 0) {
    if (const auto args_map =
            std::get_if<flutter::EncodableMap>(method_call.arguments())) {
      const auto& args = *args_map;
      const auto path = std::get_if<std::string>(&args.at("path"));
      const auto print_backgrounds =
          std::get_if<bool>(&args.at("printBackgrounds"));
      const auto margin_in_inches =
          std::get_if<double>(&args.at("marginInInches"));
      const auto page_width_mils = std::get_if<int>(&args.at("pageWidthMils"));
      const auto page_height_mils =
          std::get_if<int>(&args.at("pageHeightMils"));
      if (path && print_backgrounds && margin_in_inches && page_width_mils &&
          page_height_mils) {
        std::shared_ptr<flutter::MethodResult<flutter::EncodableValue>>
            shared_result = std::move(result);
        webview_->PrintToPdf(
            *path, *print_backgrounds, *margin_in_inches,
            static_cast<double>(*page_width_mils),
            static_cast<double>(*page_height_mils),
            [shared_result](bool success, const std::string& error_message) {
              if (success) {
                shared_result->Success();
              } else {
                shared_result->Error(kErrorNotSupported, error_message);
              }
            });
        return;
      }
