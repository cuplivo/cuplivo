...
import 'package:Cuplivo/utils/export_image_capture.dart';
...
void main() {
  ...
  test('export capture keeps medium-long images on the whole-capture path', () {
    expect(
      shouldUseFullExportCapture(const Size(480, 2665), pixelRatio: 3),
      isTrue,
    );
    expect(
      exportFullCapturePixelRatio(
        const Size(480, 2665),
  ...
  test('export capture downscales very long whole captures before slicing', () {
    final pixelRatio = exportFullCapturePixelRatio(
      const Size(480, 7537),
      requestedPixelRatio: 3,
    );
    ...
    expect(
      shouldUseFullExportCapture(const Size(480, 8000), pixelRatio: 3),
      isFalse,
    );
  });

  test('export capture slice height stays on logical pixel boundaries', () {
    final logicalHeight = exportCaptureSliceLogicalHeight(pixelRatio: 3);
  ...
  test(
    'captured export png processing leaves a single capture untouched without trim',
    () async {
      final pngBytes = _blankPaddedPng(
        width: 12,
        height: 24,
      ...
      final processed = await processCapturedExportPngForTesting(
        singlePngBytes: pngBytes,
        preservePadding: null,
      );

      expect(processed, pngBytes);
    },
  );
