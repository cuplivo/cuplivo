...
`src/lib/cuplivo/settings.ts`：输出 settings.json = 源 settings.json 逐字复制，仅对 `assistants_v1` 做三处有限手术：
...
3. Kelivo 独有字段（autoOrganizeMemory 等）原样保留——Cuplivo 忽略未知键，不崩溃
...
- 手术只针对源码核实的**静默丢失**点（Cuplivo fromJson 默认值/忽略未知键行为），共 2 类；其余键 Cuplivo restore 均宽容写入。
