...
import '../../core/services/sync/lan_sync_restore_service.dart';
import '../../core/services/sync/restore_progress.dart';
import '../../core/services/backup/incremental_apply_service.dart';
import '../../core/database/business_repository.dart';
import '../../core/database/business_restore_service.dart';
...
class _LanSyncSectionState extends State<LanSyncSection> {
  ...
  late final LanSyncRestoreService _restoreService;
  ...
  void initState() {
    ...
    Future<List<String>> assistantIds() async =>
        context.read<AssistantProvider>().assistants.map((a) => a.id).toList();
    _restoreService = LanSyncRestoreService(
      chatService: chatService,
      dataSync: _dataSync,
      applyService: IncrementalApplyService(chatService: chatService),
      businessRestore: (settings) => BusinessRestoreService(
  ...
  Future<void> _restoreAndRestart(
    ...
    try {
      ...
      final precedence = resolveSyncPrecedence(
        priority,
        isInitiator: !isServer,
      );
      await _restoreService.applyZip(
        zipFile,
        incomingWinsConflicts: precedence != ConflictPrecedence.localWins,
