...
import '../services/mcp/mcp_connection_state.dart';
...
class McpProvider extends ChangeNotifier {
  ...
  McpStatus statusFor(String id) {
    final conn = _connections[id];
    if (conn == null) return McpStatus.idle;
    return switch (conn.state) {
      McpConnectionIdle() => McpStatus.idle,
      McpConnectionConnecting() => McpStatus.connecting,
      McpConnectionConnected() => McpStatus.connected,
      McpConnectionError() => McpStatus.error,
    };
  }

  String? errorFor(String id) => _connections[id]?.errorMessage;
  ...
  List<McpServerConfig> get connectedServers => _servers
      .where((s) => _connections[s.id]?.isConnected ?? false)
  ...
  // ========================================================================
  // Connection Management (delegated to McpConnection)
  // ========================================================================
  ...
  Future<void> connect(String id) async {
    ...
    if (_connections.containsKey(id) && _connections[id]!.isConnected) return;
    ...
    // Propagate state changes as ChangeNotifier updates
    _stateSubscriptions[id]?.cancel();
    _stateSubscriptions[id] = conn.stateStream.listen((_) {
      try {
        notifyListeners();
      } catch (_) {
        // Ignore if disposed
  ...
  Future<void> disconnect(String id) async {
    _stateSubscriptions.remove(id)?.cancel();
  ...
  Future<mcp.CallToolResult?> callTool(
    ...
    return await conn.callTool(toolName, normalized);
  }
