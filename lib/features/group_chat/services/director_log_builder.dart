...
class DirectorLogBuilder {
  ...
  List<DirectorLogEntry> build({
    ...
    // per-message tool-card lookup runs once per build instead of once per
      ...
      if (message.role == 'user') {
        ...
        entries.add(
          DirectorLogEntry(
            ...
            observedAssistantId: nextAssistant?.senderId,
  ...
  List<Map<String, dynamic>> _buildApiMessages({
    ...
    // The boundary counts logical slots of the full transcript; the entry only
    // shows a prefix of them, so clamp it to what this prefix actually holds.
