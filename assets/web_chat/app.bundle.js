(() => {
  ...
  function receiveTransferChunk(chunk) {
    ...
    if (!Number.isInteger(chunk.index) || !Number.isInteger(chunk.total) || chunk.index < 0 || chunk.index >= chunk.total || chunk.total < 1) {
  ...
  function reduceEnvelope(state2, envelope) {
    if (envelope.type === "snapshot") {
      if (envelope.protocolVersion !== PROTOCOL_VERSION || envelope.assetVersion !== ASSET_VERSION) {
        throw new Error("snapshot_version_mismatch");
      }
      if (state2 && state2.renderSessionId === envelope.renderSessionId && Number(envelope.renderRevision) < Number(state2.renderRevision)) {
        return state2;
      }
      if (state2?.renderSessionId === envelope.renderSessionId) {
        const liveById = new Map((state2.messages ?? []).map((message) => [message.id, message]));
        return {
          ...envelope,
          media: { ...state2.media ?? {}, ...envelope.media ?? {} },
          messages: (envelope.messages ?? []).map((message) => {
            const live = liveById.get(message.id);
            let next = message;
            if (live?.isStreaming && message.isStreaming === true && Number.isInteger(live.streamRevision)) {
              next = {
                ...message,
            ...
            if (message.translationStreaming === true && live?.patchKind === "translation" && Number.isInteger(live.streamRevision)) {
              next = {
                ...next,
    ...
    if (envelope.type === "messagePatches") {
      if (!state2 || envelope.renderSessionId !== state2.renderSessionId || envelope.conversationId !== state2.conversationId) return state2;
      const byId = new Map(envelope.patches.map((patch) => [patch.id, patch]));
      return {
        ...state2,
        messages: state2.messages.map((message) => {
          ...
          if (Number.isInteger(revision) && message.isStreaming !== true && patch.isStreaming === true) return message;
          if (Number.isInteger(revision) && Number.isInteger(currentRevision) && revision <= currentRevision) return message;
  ...
  function visibleRange({ heights: heights2, scrollTop, viewportHeight, overscan = 700 }) {
    const lower = Math.max(0, scrollTop - overscan);
    const upper = scrollTop + viewportHeight + overscan;
    let offset = 0;
    let start = 0;
    while (start < heights2.length && offset + heights2[start] < lower) {
      offset += heights2[start];
      start += 1;
    }
    let end = start;
    let cursor = offset;
    while (end < heights2.length && cursor < upper) {
      cursor += heights2[end];
      end += 1;
    }
    return { start, end, top: offset, bottom: heights2.slice(end).reduce((a, b) => a + b, 0) };
  }
  ...
  function virtualCoverage({
    ...
    const values = Array.isArray(heights2) ? heights2.map((value) => {
    ...
    const max = end === values.length ? maxExtent : Math.max(min, Math.min(maxExtent, renderedBottom - viewport));
  ...
  function createVirtualWindowCoordinator({
  ...
  function buildWithFrameBudget({
    ...
    const budget = Number.isFinite(Number(budgetMs)) && Number(budgetMs) > 0 ? Number(budgetMs) : 6;
  ...
  function longestStablePrefix(previous, next) {
  ...
  function safeUtf16SliceEnd(text, requestedEnd) {
    ...
    if (end > 0 && end < source.length) {
      ...
      if (previous >= 55296 && previous <= 56319 && next >= 56320 && next <= 57343) end += 1;
    }
    return end;
  }
  function createAdaptiveStreamPresenter({
    schedule = requestAnimationFrame,
    now = () => performance.now(),
    commit,
    frameInterval = 32,
  ...
  function messageIndexAtOffset(heights2, scrollTop, fallbackHeight = 170) {
    if (!Array.isArray(heights2) || heights2.length === 0) return -1;
    const target = Number.isFinite(Number(scrollTop)) ? Math.max(0, Number(scrollTop)) : 0;
    const fallback = Number.isFinite(Number(fallbackHeight)) && Number(fallbackHeight) > 0 ? Number(fallbackHeight) : 170;
    let offset = 0;
    for (let index = 0; index < heights2.length; index += 1) {
      const rawHeight = Number(heights2[index]);
      const height = Number.isFinite(rawHeight) && rawHeight > 0 ? rawHeight : fallback;
      if (target < offset + height) return index;
      offset += height;
    }
    return heights2.length - 1;
  }
  function normalizeContentInset(value, fallback = 8) {
    return typeof value === "number" && Number.isFinite(value) && value >= 0 ? value : fallback;
  }
  function partitionThinkingSteps(steps, retainedCount = 2) {
    const retained = Number.isInteger(retainedCount) && retainedCount >= 0 ? retainedCount : 2;
  ...
  function normalizeMeasuredHeight(value) {
    const height = Number(value);
    return Number.isFinite(height) && height > 0 ? Math.ceil(height) : null;
  }
  function commitPendingMeasurements(committed, pending) {
  ...
  function verticalGestureIntent({
    startX,
    startY,
    currentX,
    currentY,
    ...
    const threshold = Number.isFinite(Number(slop)) && Number(slop) > 0 ? Number(slop) : 18;
    if (![dx, dy].every(Number.isFinite) || Math.hypot(dx, dy) < threshold) {
      return "hold";
    }
    return Math.abs(dy) >= Math.abs(dx) ? "vertical" : "horizontal";
  }
  function mountCodeBlock({ pre, block, header, body }) {
    pre.replaceWith(block);
    body.append(pre);
    block.append(header, body);
  }
  function formatReasoningElapsed(startAt, finishedAt, loading, now = Date.now()) {
    const start = parseTimestamp(startAt);
    if (!Number.isFinite(start)) return "";
    const finished = parseTimestamp(finishedAt);
    const end = Number.isFinite(finished) ? finished : loading ? Number(now) : start;
    if (!Number.isFinite(end)) return "";
    return `(${(Math.max(0, end - start) / 1e3).toFixed(1)}s)`;
  }
  ...
  function createExpansionCoordinator() {
    const entries = /* @__PURE__ */ new Map();
    ...
    function reconcile(key, authoritative) {
      ...
      if (entry.inFlight == null && entry.awaitingTarget == null && entry.desired === entry.authoritative) {
        entries.delete(key);
        return null;
      }
      return entry;
  ...
  function captureAnchor(container, { granular = true } = {}) {
  ...
  function restoreAnchor(container, anchor) {
    ...
    const message = messageSlots(container).find((item) => item.dataset.messageId === anchor.id);
    if (!message) return false;
    const granular = anchor.key == null ? null : [...message.querySelectorAll("[data-viewport-anchor-key]")].find((item) => item.dataset?.viewportAnchorKey === anchor.key);
    const node = granular ?? message;
    const offset = granular == null && anchor.key != null ? Number(anchor.messageOffset) : Number(anchor.offset);
    if (!Number.isFinite(offset)) return false;
    const top = container.getBoundingClientRect().top;
    container.scrollTop += node.getBoundingClientRect().top - top - offset;
    return true;
  }
  function captureInteractionAnchor(container, element) {
    const message = element?.closest?.('[data-message-slot="true"]') ?? element?.closest?.("[data-message-id]");
    const disclosure2 = element?.closest?.("[data-expansion-key]");
    const messageId = message?.dataset?.messageId;
    const expansionKey = disclosure2?.dataset?.expansionKey;
  ...
  function restoreInteractionAnchor(container, anchor) {
    if (!anchor) return false;
    const message = messageSlots(container).find((node) => node.dataset?.messageId === anchor.messageId);
    const disclosure2 = [...message?.querySelectorAll?.("[data-expansion-key]") ?? []].find((node) => node.dataset?.expansionKey === anchor.expansionKey);
    const element = disclosure2?.querySelector?.('[data-interaction-anchor="true"]') ?? disclosure2?.querySelector?.(".disclosure-header");
  ...
  function captureViewport(container, { preserve = true } = {}) {
  ...
  function restoreViewport(container, viewport) {
    ...
    const scrollHeight = Number.isFinite(rawScrollHeight) ? Math.max(0, rawScrollHeight) : 0;
    const clientHeight = Number.isFinite(rawClientHeight) ? Math.max(0, rawClientHeight) : 0;
    container.scrollTop = Math.min(scrollTop, Math.max(0, scrollHeight - clientHeight));
  }
  function viewportForSavedAnchor({
    messageIds,
    heights: heights2,
    anchor,
    viewportHeight
    ...
    const offset = Number(anchor?.offset);
    if (typeof messageId !== "string" || messageId.length === 0 || !Number.isFinite(offset) || !Array.isArray(messageIds) || !Array.isArray(heights2) || messageIds.length !== heights2.length) {
      return null;
    }
    const index = messageIds.indexOf(messageId);
    if (index < 0) return null;
    const scrollTop = heights2.slice(0, index).reduce((sum, value) => {
      const height = Number(value);
  ...
  function createFrameCoalescer(callback, schedule = requestAnimationFrame) {
    let pending = false;
    return () => {
      if (pending) return;
      pending = true;
  ...
  function createRenderCommitCoordinator({
    schedule = requestAnimationFrame,
  ...
  function createRenderGate(dispatch) {
    let blocked = false;
    let pending = false;
    return {
      request() {
  ...
  function createViewportNavigationCoordinator({
    schedule = requestAnimationFrame,
    setRenderBlocked: setRenderBlocked2,
    requestRender: requestRender2,
    settleFrames = 3
  }) {
    let revision = 0;
    let active = false;
    const frameCount = Number.isInteger(settleFrames) && settleFrames > 0 ? settleFrames : 3;
    return {
      run(settle, onComplete = null) {
        const request = ++revision;
        active = true;
        setRenderBlocked2(true);
        requestRender2();
        setRenderBlocked2(false);
        setRenderBlocked2(true);
  ...
  var backgroundLayer = document.getElementById("chat-background");
  var virtualWindowLoader = document.getElementById("virtual-window-loader");
  var virtualWindowLoaderLabel = document.getElementById("virtual-window-loader-label");
  var maxHtmlPreviewCodeUnits = 1024 * 1024;
  var state = null;
  var requestSequence = 0;
  var heights = /* @__PURE__ */ new Map();
  var pendingMeasuredHeights = /* @__PURE__ */ new Map();
  var pendingActions = /* @__PURE__ */ new Map();
  var pendingMedia = /* @__PURE__ */ new Set();
  var localExpansions = /* @__PURE__ */ new Map();
  var mountedSlots = /* @__PURE__ */ new Map();
  var presentedStreamContent = /* @__PURE__ */ new Map();
  var pendingMountedUpdates = /* @__PURE__ */ new Set();
  var markdownHtmlCache = /* @__PURE__ */ new Map();
  var streamingMarkdownStates = /* @__PURE__ */ new WeakMap();
  var staticMarkdownStates = /* @__PURE__ */ new WeakMap();
  var streamStructureSignatureCache = /* @__PURE__ */ new WeakMap();
  var expansionCoordinator = createExpansionCoordinator();
  var disclosureAnimations = /* @__PURE__ */ new WeakMap();
  ...
  var scrollStopTop = 0;
  var scrollStopLeft = 0;
  var gestureActive = false;
  var gestureIntent = "idle";
  var renderedSessionId = null;
  var renderedConversationId = null;
  var reasoningElapsedTimer = null;
  var navigationCursorId = null;
  var navigationEdge = null;
  var forcedViewport = null;
  var virtualPrefetchFrame = 0;
  var virtualPrefetchTarget = null;
  var virtualPrefetchActive = false;
  var virtualPrefetchRevision = 0;
  var virtualPruneTimer = null;
  var virtualWindowLoading = false;
  var lastTimelineScrollTop = 0;
  var programmaticNavigationActive = false;
  var measuredHeightsPending = false;
  var pendingInteractionAnchor = null;
  var interactionMeasureFrame = 0;
  var interactionRevision = 0;
  var interactionExpiryTimer = null;
  var bottomHoldTimer = null;
  var initialBottomPin = false;
  var rendererLoads = /* @__PURE__ */ new Map();
  var readyRenderers = /* @__PURE__ */ new Set();
  var pendingRendererRenders = /* @__PURE__ */ new Map();
  var pendingRendererRefreshes = /* @__PURE__ */ new Map();
  var pendingMermaidCommits = /* @__PURE__ */ new Map();
  var rendererRefreshFrame = 0;
  var bridge = {
    post(message) {
  ...
  var renderCommitCoordinator = createRenderCommitCoordinator({
    commit: (identity) => {
      if (!state || state.renderSessionId !== identity.renderSessionId || state.conversationId !== identity.conversationId || state.renderRevision !== identity.renderRevision) return;
      bridge.post({
  ...
  var safeProtocolDiagnostics = /* @__PURE__ */ new Set([
    "protocol_mismatch",
  ...
  var transparentRemoteImage = "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==";
  ...
  function messageHeight(message) {
    return heights.get(message.id) ?? 170;
  }
  function isMediaHandle(value) {
    return value?.startsWith("local:") || value?.startsWith("asset:") || value?.startsWith("remote:");
  }
  ...
  function markdownSourceForRender(content) {
    ...
    return String(content ?? "").replace(/(!\[[^\]]*\]\(\s*<?)(http:\/\/[^\s>)]+)/gi, replace).replace(/(<img\b[^>]*\bsrc\s*=\s*["'])(http:\/\/[^"']+)/gi, replace);
  }
  ...
  function rerenderWhenRendererReady(renderer, promise, root) {
    ...
    const key = `${renderer}:${metadata?.renderSessionId ?? "session"}:${metadata?.messageId ?? "timeline"}:${metadata?.anchorKey ?? "root"}`;
  ...
  function appearanceDataKey(surface, suffix) {
    return `style${surface[0].toUpperCase()}${surface.slice(1)}${suffix.split("-").map((part) => `${part[0].toUpperCase()}${part.slice(1)}`).join("")}`;
  }
  function appearanceCssValue(spec, value) {
    if (spec.kind === "color") {
      return typeof value === "string" && /^#[0-9a-f]{6}(?:[0-9a-f]{2})?$/i.test(value) ? value : null;
    }
    if (typeof value !== "number" || !Number.isFinite(value) || value < spec.min || value > spec.max) return null;
    if (spec.kind === "shadow") {
      if (value === 0) return "none";
      const y = Number((value * 0.5).toFixed(2));
      const blur = Number((value * 1.5).toFixed(2));
      const alpha = Number(Math.min(0.28, 0.08 + value * 8e-3).toFixed(3));
  ...
  function applyTheme() {
    ...
    const backgroundOwner = state.display?.backgroundOwner === "flutter" ? "flutter" : "web";
    ...
    if (backgroundOwner === "web") {
      ...
      backgroundLayer.style.backgroundImage = isImage ? `url("${source.replaceAll('"', "%22")}")` : "none";
  ...
  function sendAction(action, messageId = null, payload = {}) {
    ...
    if ((action === "loadMoreBefore" || action === "loadMoreAfter") && [...pendingActions.values()].includes(action)) return null;
  ...
  var iconCodepoints = Object.freeze({
    copy: 57502,
    more: 57526,
    regenerate: 57669,
    speak: 57771,
  ...
  function updateDisclosure(root, header, body, expanded, onLayoutSettled = null) {
    ...
    const animation = body.animate(
      expanded ? [{ opacity: 0, transform: "translateY(-4px)" }, { opacity: 1, transform: "translateY(0)" }] : [{ opacity: 1, transform: "translateY(0)" }, { opacity: 0, transform: "translateY(-4px)" }],
  ...
  function patchStreamingMarkdownRoot(root, source, kind) {
    const markdownEnabled = kind === "user" ? state.display?.userMarkdown !== false : kind === "reasoning" ? state.display?.reasoningMarkdown !== false : state.display?.assistantMarkdown !== false;
    if (!markdownEnabled) {
    ...
    const signatures = tokens.map((token) => `${token.type}\0${token.raw ?? ""}`);
    ...
    for (let index = prefix; index < tokens.length; index += 1) {
      ...
      container.dataset.viewportGroupKey = `${root.dataset.viewportGroupKey}:token:${index}`;
  ...
  function markdownNode(content, streaming = false, kind = "assistant", messageId = null, anchorKey = null) {
    const root = document.createElement("div");
    root.className = "markdown";
    if (messageId) root.dataset.messageId = messageId;
    const source = content ?? "";
    const resolvedAnchorKey = anchorKey ?? `${messageId ?? "timeline"}:${stableTextKey(`markdown:${kind}`, source)}`;
    ...
    try {
      ...
      const cacheKey = !streaming && source.length <= 64 * 1024 ? stableTextKey(`markdown:${kind}`, source) : null;
  ...
  function enhanceMarkdown(root, streaming, messageId = null) {
    ...
    for (const image of root.querySelectorAll("img")) {
      ...
      const handle = state.remoteMediaHandles?.[source] ?? remoteHandleFromPlaceholder(source);
    ...
    try {
      ...
      bridge.post({ type: "diagnostic", code: "math_block_failed" });
    }
  }
  function enhanceRawCitations(root, messageId) {
    ...
    for (const node of textNodes) {
      ...
      for (const match of source.matchAll(pattern)) {
        const references = String(match[1]).split(",").map((part) => parseCitationReference(part.trim()));
  ...
  function parseCitationReference(raw) {
    ...
    if (!/^[A-Za-z0-9_-]+$/.test(index) || separator >= 0 && !/\d/.test(index) || !id) return null;
    return { index, id };
  }
  ...
  function ensureReasoningElapsedTimer() {
    ...
    if (reasoningElapsedTimer == null && document.querySelector('[data-reasoning-elapsed][data-reasoning-loading="true"]')) {
  ...
  function renderReasoningSegment(message, segment, index, parent) {
    ...
    const expanded = kind === "legacy" ? localExpansion(key, authoritative) : expansionCoordinator.value(key, authoritative);
    const body = document.createElement("div");
  ...
  function groupChainCards(parent) {
    ...
    for (const child of [...parent.children]) {
      const isStep = child.dataset?.component === "reasoning" || child.dataset?.component === "tool";
      if (!isStep) {
        chain = null;
        continue;
      }
  ...
  function renderTool(message, tool, parent) {
    ...
    const summary = state.display?.showToolResultSummary === true && tool.content != null ? String(tool.content).replace(/\s+/g, " ").trim().slice(0, 72) : "";
    const label = `${tool.content == null ? t("toolCall") : t("toolResult")} ${tool.toolName}${summary ? ` \xB7 ${summary}` : ""}`.trim();
    const body = document.createElement("div");
  ...
  function applyThinkingStepCollapse(message, parent) {
    ...
    for (const [cardIndex, card] of [...parent.querySelectorAll(":scope > .chain-card")].entries()) {
      const steps = [...card.children].filter((node) => node.dataset?.component === "reasoning" || node.dataset?.component === "tool");
      ...
      const applyExpanded = (expanded) => {
        ...
        const label = expanded ? t("collapseThinkingSteps") : formatCountTemplate(
  ...
  function renderAttachments(message, parent) {
    for (const attachment of message.attachments ?? []) {
      ...
      const handle = isMediaHandle(attachment.reference) ? attachment.reference : state.remoteMediaHandles?.[attachment.reference];
      if (attachment.kind === "image" && /^(data:|https?:)/i.test(source)) {
        ...
        if (handle) {
          file.addEventListener("click", () => sendAction("openAttachment", message.id, { handle }));
        } else {
          file.disabled = true;
        }
        parent.append(file);
  ...
  function renderAvatar(message) {
    ...
    if (isUser) {
      const image2 = mediaImage(user.avatar, user.name, false);
      if (image2) avatar.append(image2);
      else if (user.avatarLabel) avatar.textContent = user.avatarLabel;
      else avatar.append(iconNode("user"));
      return avatar;
    }
    if (assistant.useAvatar) {
      const image2 = mediaImage(assistant.avatar, assistant.name, false);
      if (image2) avatar.append(image2);
  ...
  function renderMessageHeader(message) {
    ...
    if (isUser && display.showUserName !== false || !isUser && display.showModelName !== false) {
      ...
      name.className = "name";
      name.textContent = isUser ? state.user?.name ?? t("user") : assistant.useName ? assistant.name || t("assistant") : message.modelId || t("assistant");
      meta.append(name);
    }
  ...
  function renderMessageActions(message) {
    ...
    for (const action of message.actions ?? []) {
      const icon = action === "speak" && state.display?.ttsActive === true ? "stop" : {
        copy: "copy",
        edit: "edit",
        resend: "resend",
        regenerate: "regenerate",
    ...
    if ((message.versionCount ?? 1) > 1) {
      const previous = iconButton(
        "previous",
        t("previousVersion"),
        () => sendAction("version", message.id, { delta: -1 }),
        "version-action"
      );
  ...
  function renderMessage(message, isLast = false) {
    ...
    const actions = message.selecting || message.role !== "user" && message.isStreaming ? null : renderMessageActions(message);
    if (actions) actions.dataset.viewportAnchorKey = `${message.id}:actions`;
    const suggestions = message.role !== "user" && isLast && !message.isStreaming ? renderSuggestions() : null;
    main.append(
      ...[
        header,
  ...
  function reportSlowRender(startedAt, code) {
    ...
    bridge.post({
      type: "diagnostic",
      code,
      durationBucketMs: Math.min(1e3, Math.ceil(elapsed / 50) * 50)
    });
  }
  ...
  function render() {
    ...
    const savedViewport = sameSession ? null : viewportForSavedAnchor({
    ...
    const missingSavedAnchor = !sameSession && state.initialViewportAnchor != null && savedViewport == null;
    ...
    resizeObserver ?? (resizeObserver = new ResizeObserver(handleMeasuredHeights));
    const fragment = document.createDocumentFragment();
    const observedSlots = [];
  ...
  function isCurrentInteraction(anchor) {
    return anchor != null && pendingInteractionAnchor != null && anchor.revision === interactionRevision && pendingInteractionAnchor.revision === anchor.revision;
  }
  ...
  function restorePreferredViewport(viewport, allowInteractionRelease = false, dropMissingInteraction = false) {
    if (pendingInteractionAnchor) {
      if (restoreInteractionAnchor(timeline, pendingInteractionAnchor)) {
        if (allowInteractionRelease && interactionAnchorCanRelease(pendingInteractionAnchor)) {
  ...
  function queueInteractionLayoutReconcile(anchor, releaseRequested = false) {
    if (!isCurrentInteraction(anchor)) return;
    anchor.releaseRequested || (anchor.releaseRequested = Boolean(releaseRequested));
    ...
    interactionMeasureFrame = requestAnimationFrame(() => {
      ...
      const slot = activeAnchor == null ? null : mountedSlots.get(activeAnchor.messageId);
      if (slot?.isConnected) {
        stageMeasuredHeight(
  ...
  function reconcileMeasuredHeights() {
    if (!measuredHeightsPending && !pendingInteractionAnchor || !state?.messages?.length || touchActive || userScrolling || virtualWindowLoading || !topSpacer || !bottomSpacer || renderedRange.start < 0) return;
    const viewport = captureViewport(timeline);
    const changed = measuredHeightsPending ? commitPendingMeasurements(heights, pendingMeasuredHeights) : false;
    ...
    const messages = state.messages;
    topSpacer.style.height = `${messages.slice(0, renderedRange.start).reduce((sum, message) => sum + messageHeight(message), 0)}px`;
    bottomSpacer.style.height = `${messages.slice(renderedRange.end).reduce((sum, message) => sum + messageHeight(message), 0)}px`;
    restorePreferredViewport(viewport, true);
  ...
  var scheduleRender = createFrameCoalescer(renderSafely);
  var scheduleMeasuredHeightReconcile = createFrameCoalescer(
    reconcileMeasuredHeights
  );
  var renderGate = createRenderGate(scheduleRender);
  function requestRender() {
    renderGate.request();
  }
  function setRenderBlocked(value) {
    renderGate.setBlocked(value);
  }
  var streamPresenter = createAdaptiveStreamPresenter({
  ...
  var sendViewportMetrics = createFrameCoalescer(() => {
  ...
  var viewportNavigation = createViewportNavigationCoordinator({
    setRenderBlocked,
    requestRender
  });
  ...
  function messageOrderMatches(messageIds, renderSessionId) {
    ...
    return state?.renderSessionId === renderSessionId && current.length === messageIds.length && current.every((message, index) => message.id === messageIds[index]);
  }
  async function renderVirtualTarget(scrollTop, requestRevision) {
    ...
    const built = await buildWithFrameBudget({
      items: indices,
      shouldContinue: () => virtualWindowCoordinator.isCurrent(requestRevision) && messageOrderMatches(messageIds, renderSessionId),
      build: (index) => {
    ...
    if (!virtualWindowCoordinator.isCurrent(requestRevision) || !messageOrderMatches(messageIds, renderSessionId)) {
  ...
  var virtualWindowCoordinator = createVirtualWindowCoordinator({
    ...
    settleTarget: (target) => {
      ...
      const restoredInteraction = pendingInteractionAnchor != null && restoreInteractionAnchor(timeline, pendingInteractionAnchor);
      if (restoredInteraction && interactionAnchorCanRelease(pendingInteractionAnchor)) {
        releaseInteractionAnchor(pendingInteractionAnchor);
      }
    ...
    onError: (error) => {
      const code = error?.message === "virtual_window_timeout" ? "virtual_window_timeout" : error?.message === "virtual_window_target_missing" ? "virtual_window_target_missing" : "virtual_window_render_failed";
  ...
  async function extendVirtualRange(targetTop, prefetchRevision) {
    if (!state?.messages?.length || virtualWindowLoading || pendingInteractionAnchor || !topSpacer || !bottomSpacer) {
      return false;
    }
    const messages = state.messages;
    const renderSessionId = state.renderSessionId;
    ...
    const built = await buildWithFrameBudget({
      items: indices,
      shouldContinue: () => prefetchRevision === virtualPrefetchRevision && !virtualWindowLoading && messageOrderMatches(messageIds, renderSessionId) && renderedRange.start === baseRange.start && renderedRange.end === baseRange.end,
      build: (index) => {
        const message = state.messages[index];
        if (message?.id !== messageIds[index]) {
          throw new Error("virtual_window_stale");
    ...
    if (prefetchRevision !== virtualPrefetchRevision || virtualWindowLoading || !messageOrderMatches(messageIds, renderSessionId) || renderedRange.start !== baseRange.start || renderedRange.end !== baseRange.end) {
      throw new Error("virtual_window_stale");
    }
    const viewport = captureViewport(timeline);
    ...
    if (before.childNodes.length) {
      const preset = start === 0 && !timeline.querySelector('[data-component="preset-toggle"]') ? createPresetNode() : null;
    ...
    topSpacer.style.height = `${messages.slice(0, start).reduce((sum, message) => sum + messageHeight(message), 0)}px`;
    bottomSpacer.style.height = `${messages.slice(end).reduce((sum, message) => sum + messageHeight(message), 0)}px`;
    restoreViewport(timeline, viewport);
  ...
  function scheduleVirtualPrefetch(targetTop) {
    ...
    virtualPrefetchFrame = requestAnimationFrame(async () => {
      ...
      try {
        ...
        } else if (extended && !touchActive && !userScrolling && !virtualWindowLoading) {
          scheduleVirtualPrune();
        }
      }
    });
  ...
  function pruneVirtualRange() {
    if (!state?.messages?.length || touchActive || userScrolling || virtualWindowLoading || virtualPrefetchActive || pendingInteractionAnchor || !topSpacer || !bottomSpacer) return;
    const messages = state.messages;
    const viewport = captureViewport(timeline);
    const desired = visibleRange({
      heights: messages.map(messageHeight),
  ...
  timeline.addEventListener("touchmove", (event) => {
    ...
    if (touchStartX == null || touchStartY == null || currentX == null || current == null) return;
    const intent = verticalGestureIntent({
      startX: touchStartX,
      startY: touchStartY,
      currentX,
  ...
  timeline.addEventListener("keydown", (event) => {
    if (event.key === "Home") {
      ...
      markUserScroll();
      timeline.scrollBy({ top: -timeline.clientHeight * 0.85, behavior: "smooth" });
    } else if (event.key === "PageDown") {
      markUserScroll();
      timeline.scrollBy({ top: timeline.clientHeight * 0.85, behavior: "smooth" });
    } else if (event.key === "ArrowUp" || event.key === "ArrowDown" || event.key === " " || event.key === "Spacebar") {
      markUserScroll();
    }
  });
  ...
  function handleViewportCommand(envelope) {
    ...
    if (command === "top") {
      ...
      const duration = Math.max(0, Math.min(2e3, Number(payload.durationMs) || 0));
  ...
  function navigateToEdge(edge, { hold = false } = {}) {
    programmaticNavigationActive = true;
    navigationEdge = edge;
    const settle = edge === "top" ? () => timeline.scrollTo({ top: 0, behavior: "auto" }) : () => timeline.scrollTo({ top: timeline.scrollHeight, behavior: "auto" });
    const complete = () => {
  ...
  function handleMessagePatches(envelope) {
    if (!state || envelope.renderSessionId !== state.renderSessionId || envelope.conversationId !== state.conversationId) return;
    ...
    for (const patch of patches) {
      ...
      if (Number.isInteger(patch.streamRevision) && next.streamRevision !== patch.streamRevision) {
      ...
      const structuralChange = streamStructureSignature(previous) !== streamStructureSignature(next);
      if (!contentChanged || structuralChange) updateMountedMessage(id);
    }
  }
  function handleMediaResult(payload) {
    if (!state || payload.renderSessionId !== state.renderSessionId || payload.conversationId !== state.conversationId) return;
    state = {
      ...state,
    ...
    for (const message of state.messages ?? []) {
      const affected = message.modelIcon === payload.handle || userAvatar && message.role === "user" || assistantAvatar && message.role !== "user" || (message.attachments ?? []).some(
      ...
      try {
        ...
        if (envelope.type === "transferChunk") {
          ...
          if (payload.type === "mediaResult") {
            ...
            if (previousSessionId && (previousSessionId !== state?.renderSessionId || previousConversationId !== state?.conversationId)) {
              viewportNavigation.cancel();
              virtualWindowCoordinator.cancel();
        ...
        } else if (envelope.type === "mediaError" && state && envelope.renderSessionId === state.renderSessionId && envelope.conversationId === state.conversationId) {
