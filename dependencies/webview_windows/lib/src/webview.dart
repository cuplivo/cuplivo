...
class WebviewController extends ValueNotifier<WebviewValue> {
  ...
  /// Prints the currently loaded document to a PDF file at [path].
  ///
  /// [marginInInches] is applied to all four page edges (14 mm ≈ 0.551 in).
  /// [pageWidthMils] / [pageHeightMils] are the paper size in thousandths of
  /// an inch: A4 = 8268 × 11693, US Letter = 8500 × 11000.
  Future<void> printToPdf(
    String path, {
    bool printBackgrounds = true,
    double marginInInches = 0.551,
    int pageWidthMils = 8268,
    int pageHeightMils = 11693,
  }) async {
    if (_isDisposed) {
      return;
    }
    assert(value.isInitialized);
    return _methodChannel.invokeMethod('printToPdf', <String, dynamic>{
      'path': path,
      'printBackgrounds': printBackgrounds,
      'marginInInches': marginInInches,
      'pageWidthMils': pageWidthMils,
      'pageHeightMils': pageHeightMils,
    });
  }
